...
from utils.utils import create_val_loggers, create_optimizers, create_schedulers, resume_training_setup, \
    setup_distributed_experiment, get_loss_fn, resume_training_setup_v2


@hydra.main(config_path='configs/segformer_config.yaml', strict=False)
def main(configs):
...
def main_process(gpu, configs, is_distributed_training=True):
    ...
    if is_distributed_training:
        print("Not converting to SYNCBN for segformer")
        # model = nn.SyncBatchNorm.convert_sync_batchnorm(model)
    model = model.cuda(gpu)

    # comment if using Zero Optimizer
    ...
    if configs.resume_training:
        ...
        start_epoch = resume_training_setup_v2(configs, model, schedulers, start_step=configs.get('start_step', 0))

    model = DistributedDataParallel(model, device_ids=[gpu], find_unused_parameters=True) if is_distributed_training else model

    # Uncomment if using Zero optimizer
    # optimizers = create_optimizers(model, configs)
    # schedulers = create_schedulers(optimizers, mode=configs.lr_scheduler, num_epochs=configs.epochs,
    #                                iters_per_epoch=len(dataloaders['train']), min_lr=configs.min_lr)
