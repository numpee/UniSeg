...
class BCENullTrainer(AbstractBaseTrainer):
    ...
    def train_one_epoch(self, epoch):
        ...
        for batch_idx, ((image, target, _), domains) in enumerate(dataloader_tqdm):
            ...
            target_one_hot = self.generate_target_one_hot(target)
            self._reset_grad()

            with autocast(enabled=self.use_amp):
                outputs = self.model(image)[0]
                loss_return = self.criterion(outputs, target_one_hot)
